...
- **Memory System:** Live AI detection and saving of user context, with markers for auto-save (`[MEMORY_SAVE]`), proposals (`[MEMORY_PROPOSAL]`), and updates (`[MEMORY_UPDATE]`). The distinction between SAVE and PROPOSAL is based on whether information is **objective** (verifiable facts → auto-save) or **subjective** (opinions, preferences, attitudes → propose for confirmation). Memories are categorized into 3 categories: `ABOUT_ME` (life facts, accounts, situation), `PREFERENCES` (communication style, risk tolerance), `PRIORITIES` (goals, focus areas). A server-side fallback (`inferMissingMemoryActions`) detects personal info patterns when the AI fails to emit markers.
