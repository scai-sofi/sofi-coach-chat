...
- **Goal Proposal System:** Live AI-generated goal proposals via `[GOAL_PROPOSAL]` markers. Goal proposals create DRAFT goals in the Goals Center and send a system nudge in chat. Users confirm or dismiss from the "Suggested" section in Goals Center. Goal types: `EMERGENCY_FUND`, `SAVINGS_TARGET`, `CUSTOM` (all map to **Save up** tab), `DEBT_PAYOFF` (**Pay off** tab), `INVESTMENT` (**Investment** tab). Tab order: Save up → Pay off → Investment. The internal category key is `pay-off` (not `pay-down`). Goal statuses: `DRAFT`, `ON_TRACK`, `AT_RISK`, `PAUSED`, `COMPLETED`.
- **Persona Goal Data (`constants/personaGoals.ts`):** Each persona has a curated set of goals that reflects their financial situation and demonstrates variety across all goal types, tabs, and statuses. Maya (new member): CC payoff ON_TRACK + travel savings DRAFT. Marcus (active): EF ON_TRACK + CC payoff AT_RISK + wedding savings ON_TRACK. Jordan (debt-focused): CC payoff ON_TRACK + EF PAUSED + move-in fund CUSTOM DRAFT. David (sophisticated): EF ON_TRACK + kitchen remodel COMPLETED + Roth IRA INVESTMENT ON_TRACK + annual brokerage INVESTMENT ON_TRACK. Goals are injected into sessions via `constants/seededHistory.ts` using a per-session `goals` override field, replacing the generic scenario-level goal arrays. Both `mobile/` (GA) and `phase2/` use the same `PERSONA_GOALS` map.
