...
export function generateAIResponse(userInput: string, store: StoreState): Partial<Message> | null {
  ...
  if (input.includes('outdated') || input.includes('that dining thing')) {
    return {
      ...
      chips: [{ type: 'memory-updated', label: 'Profile updated' }],
  ...
  if (input.includes('help me pay off') || input.includes('pay off credit')) {
    return {
      ...
      goalProposal: {
        id: uid(), type: 'DEBT_PAYOFF' as GoalType, title: 'Credit Card',
  ...
  if (input.includes('what do you know') || input.includes('know about me')) {
    ...
    return {
      content: memoryList
        ? `**What I Know About You**\n\nYour Profile has ${memoryCount} active items across ${Object.keys(grouped).length} categories:\n\n${memoryList}\n\n**How I Use This**\nYour Profile helps me personalize my advice — from the level of detail I give you, to which goals I prioritize in recommendations, to how I frame financial decisions.\n\n**Your Control**\nYou can edit, pause, or remove any of these anytime in your Profile. Everything stays transparent.`
        : "**Your Profile Is Empty**\n\nI don't have any saved context about you yet, but that changes fast as we chat. Here's how it works:\n\n• **Automatic:** I'll pick up on preferences and context as we talk, always asking before saving\n• **Manual:** Tell me things directly — like \"remember that I'm saving for a house\" — and I'll store it immediately\n• **Organized by tab:** About Me, My Preferences, My Priorities, My Finances, and My Goals\n\nThe more context I have, the more tailored and useful my advice becomes. Want to tell me something about yourself to get started?",
    };
  }

  if (input.includes('how does my memory work') || input.includes('how does memory work') || input.includes('how does my profile work') || input.includes('how does profile work')) {
    return {
      content: "**How Your Profile Works**\n\nGreat question! Here's the full picture:\n\n**How I Learn**\nI pick up on your preferences, habits, and context as we chat. I'll always ask before saving something new — nothing gets stored without your awareness.\n\n**What I Track**\nYour Profile is organized into tabs:\n• **About Me** — your life situation, accounts, financial details, household info\n• **My Preferences** — how you like to receive information, risk tolerance, financial approach\n• **My Priorities** — what you're working toward, focus areas, key events\n• **My Finances** — your SoFi and linked external accounts\n• **My Goals** — tracked goals with progress, milestones, and linked products\n\n**You're Always in Control**\n• **View & edit:** Open your Profile to see, edit, pause, or remove anything\n• **Per-category control:** Toggle individual categories on or off without affecting the rest\n• **Tell me directly:** Say \"remember that...\" to save something specific, or \"forget about...\" to remove it\n\n**Why It Matters**\nThe more context I have, the more personalized and relevant my advice becomes. But your privacy always comes first — you decide what I know. Want to tell me something to get started?",
      suggestions: ['Show me my Profile', 'What do you know about me?'],
    };
  }
